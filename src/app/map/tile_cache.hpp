...
/// application, at most four downloads at a time, every tile cached), as ADR 0010 decides.
///
...
/// downloaded tile is written to disk before `tile_ready` is emitted. Offline, no download is
/// queued or started and `tile` reports tiles that are neither in memory nor on disk as
/// missing.
///
/// A tile that failed to download is not remembered: the next `tile` call for it queues it
/// again.
///
...
public:
    ...
    /// `https://tile.openstreetmap.org/{z}/{x}/{y}.png` as URL template and the bare
    /// `User-Agent` `NMEASimulatorX`. The application replaces the latter with one naming its
    /// version and project page, as the tile usage policy asks.
    ///
    ...
    ///
    /// Switching off empties the download queue. Downloads already running are not aborted:
    /// they still finish, cache their tile and emit their signal.
    ///
    ...
    /// asked for) and `tile_ready` or `tile_failed` is emitted once the reply finishes.
    ///
    ...
    /// Removes every tile from memory and disk and empties the download queue.
    ///
    /// The directory itself is recreated empty. Downloads already running are not aborted,
    /// so a tile that arrives afterwards is cached again.
    void clear();
...
signals:
    ...
    /// not an image.
    ///
    /// Nothing is cached for the tile, and a later `tile` call queues it again.
    ///
...
private:
    ...
    /// `tile_ready` or `tile_failed`, and starts the next queued download.
    ///
    ...
    /// Number of downloads running, in [0, 4].
    int active_{0};
};
